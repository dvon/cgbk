...
@.  *Put square.html, square.js and style.css in a folder, run a
    ...
    result should look like what you see in Figure 1 below.*

![A Cleared Canvas](images/screenshot_3_1.png)

## Example 2: A Red Square
...
@.  *Put the five files from this example in a folder, open
    square.html in a web browser...  Figure 2 shows what I expect
    you to see.*

![A Red Square](images/screenshot_3_2.png)
...
## Example 3:  A More Colorful Square
...
With these changes, you get the image below
(depending on the random value assigned to `blueValue`).

![A More Colorful Square](images/screenshot_3_3.png)
...
@.  *`gl.TRIANGLE_FAN` is an alternative to `gl.TRIANGLE_STRIP`.
    ...
    `gl.TRIANGLE_FAN`, modify example 2 so that it draws a
    yellow octagon.  (Make the center of the octagon the first
    vertex.)*

@.  *After doing the previous exercise, modify example 3 so that
